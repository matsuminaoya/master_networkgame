...
def Graph_plot_tc_tl_tf(csv, name, generation):
    ...
    heatmap_data = data_step.groupby(["tl","link_count"]).size().unstack(fill_value=0)
    ...
    heatmap_data = heatmap_data.reindex(index=tl_labels, columns=link_labels, fill_value=0)
    ...
    plt.savefig("pic81/"+name+"_plot4_linktl_g"+str(generation)+".png") #TODO:
    plt.close()
