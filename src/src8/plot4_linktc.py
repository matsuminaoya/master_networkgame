...
def Graph_plot_tc_tl_tf(csv, name, generation):
    ...
    heatmap_data = data_step.groupby(["tc","link_count"]).size().unstack(fill_value=0)
    ...
    heatmap_data = heatmap_data.reindex(index=tc_labels, columns=link_labels, fill_value=0)
    ...
    plt.savefig("pic81/"+name+"_plot4_linktc_g"+str(generation)+".png") #TODO:
    plt.close()
