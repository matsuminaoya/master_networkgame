#TODO:チョイスを修正、初期値揃えよう、フレキシブル化してみた、初期値は三つ別に選択にしたこれで論文書けるかも、リストappendではなくnumpyarryで最初に枠決めて高速化、他に高速化できそうなとこ探してるcalculate_cnum簡略化、linkedchoice高速化、リンクマトリクス書き換え逐次最適化切り貼り動作確認okここのミスはない、簡単に動作確認できる。グラフ作成時のコメント削除、毎ラウンド0で協力非協力が初期化されるのはおかしいから修正main2→main3、初期値をグラフに入れる、協力非協力グラフに入れる、cnumの助長を減らす、グラフクローズ←ココ、gをずらす、間違いを探す
