...
// あとで、記録する世代を100世代ごとにするとか飛ばす,フェルミ関数のベータを10に
// 1→2 逐次出力化 2→3 ファイル名長いが分かりやすく
// 3→4 resetありなし逆やったので正しくファイル名に表記
// 4→5 コメントアウトを綺麗にした
...
void part1() {
  // CSVファイルを出力するための準備
  FILE *csv_file =
      fopen("5_oror_both_null_noreset_t10_g10000_r100_w5000_b1.csv",
            "w");  // TODO:
  ...
  for (int tr = 0; tr < trial; tr++) {  // トライアル開始
    ...
    for (int ge = 0; ge < generation; ge++) {  // 世代開始
      ...
      for (int k = 0; k < NUM_AGENTS; k++) {
        ...
        float r = (float)rand() / RAND_MAX;
        if (r < prob) {
          agents[i].tc = agents[j].tc;  // TODO:「tl/tf」どちらか消す
...
// 5_oror_both_null_noreset_t10_g10000_r100_w5000_b1 終了
// 5_oror_form_null_noreset_t10_g10000_r100_w5000_b1
void part2() {
  // CSVファイルを出力するための準備
  FILE *csv_file =
      fopen("5_oror_form_null_noreset_t10_g10000_r100_w5000_b1.csv",
            "w");  // TODO:
  ...
  for (int tr = 0; tr < trial; tr++) {  // トライアル開始
    ...
    for (int ge = 0; ge < generation; ge++) {  // 世代開始
      ...
      for (int i = 0; i < NUM_AGENTS; i++) {
        if (will_learn[i] == 1) continue;
        // tc, tl, tf のそれぞれを ±0.1 変化させる
        float *traits[2] = {// TODO:「tl/tf」どちらか消す、二箇所3→2
