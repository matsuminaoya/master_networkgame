...
int main(void)
{
  ...
  int sumall[X]; /*繋がってるそのラウンドでgiveしたプレイヤー数合計 世代最初に初期化*/
  int keika;/*ラウンド数え用*/
...
ritoku[n] += (double)(b*(sumall[n])-c*givekeep[n]*(X-1));
      ...
      /*ラウンド後半　つながっていればleave,つながっていなければjoinを選択 g回行う*/
      ...
      for(countg=0;countg<g;countg++){
          ...
          if(active[playerA][playerB]==1){
              if(ikiti[playerA][1]>(double)givekeep[playerB] || ikiti[playerB][1]>(double)givekeep[playerA]){
                  active[playerA][playerB]=0;
                  active[playerB][playerA]=0;
                }
            }else{
                if(ikiti[playerA][2]<=(double)givekeep[playerB] && ikiti[playerB][2]<=(double)givekeep[playerA]){
  ...
  for(n=0;n<X;n++){
    ...
    for(n=0;n<X;n++){
      ...
      for(countg=0;countg<g;countg++){
          ...
          if(active[playerA][playerB]==1){
              if(ikiti[playerA][1]>(givekeepgoukei[playerB]/(double)(t+1)) || ikiti[playerB][1]>(givekeepgoukei[playerA]/(double)(t+1))){
                  active[playerA][playerB]=0;
                  active[playerB][playerA]=0;
                }
            }else{
                if(ikiti[playerA][2]<=(givekeepgoukei[playerB]/(double)(t+1)) && ikiti[playerB][2]<=(givekeepgoukei[playerA]/(double)(t+1))){
                        active[playerA][playerB]=1;
                        active[playerB][playerA]=1;
                            }
                        }
  ...
  for(n=0;n<X;n++){
      ...
      for(count=0;count<X;count++){
          if(active[n][hikaku]==1){
              ran = genrand_real1();
              if(ran<1/(1+exp((double)a*(averitoku[hikaku]-averitoku[n])))){
